...
- **让 task id 出现在命令行里**。行与 job 的配对用的是和"未上报的 job 行"同一套标签启发式：命令行里的 `--task sync-catalog` 认得出来，而命令行里从没出现过的 `job1` 认不出来。匹配的是**完整的词**，这个方向才是安全的：`rebuild` 不算命名了一个叫 `build` 的任务，于是那个 job 会保住自己那一行、也仍然会提醒模型；换成子串匹配，它会被悄悄算作"已上报"。
