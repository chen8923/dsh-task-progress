...
function readConfig(raw) {
	...
	const dirName = typeof record["dirName"] === "string" && DIR_NAME_RE$1.test(record["dirName"]) ? record["dirName"] : CONFIG_DEFAULTS.dirName;
	const roots = Array.isArray(record["roots"]) ? record["roots"].filter((root) => typeof root === "string" && root.length > 0) : CONFIG_DEFAULTS.roots;
	return {
		dirName,
...
function requestedSession(req) {
	const session = new URL(String(req.url ?? "/"), "http://localhost").searchParams.get("session");
...
function progressPromptText() {
	return "Long tasks can report live progress to the user: for a command you expect to run longer than about 30 seconds, have the script append one JSON event per update to `$DSH_PROGRESS_DIR/<task>.jsonl` (`{\"v\":1,\"task\":\"build\",\"state\":\"running\",\"pct\":40,\"msg\":\"linking\"}`; `state` is running/done/failed/cancelled, `done`/`total`/`unit` are optional counters), or run `node \"$DSH_PROGRESS_CLI\" emit --task build --pct 40 --msg \"linking\"` (also `done`, `fail`, `list`). Both variables are set inside every shell call. Do not read the progress file back — it is the human's view.";
}
/**
...
function apply(ctx, rawConfig) {
	const store = createTaskStore(readConfig(rawConfig));
...
//#endregion
export { CONFIG_DEFAULTS, PROGRESS_CLI_KEY, PROGRESS_DIR_KEY, PROMPT_ORDER_NAME, PROMPT_SECTION_NAME, SETTINGS_NAMESPACE, apply, createTaskStore, inject, name, progressPromptText, progressSchema, readConfig, registerProgressEnv, registerProgressPrompt, registerProgressSettings, registerStateRoute, resolveProgressSettings, stateHandler };
