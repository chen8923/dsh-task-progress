...
function isCovered(job, store, now) {
	return liveTaskNames(store, now, job.ownerSession).some((name) => labelNamesTask(job.label, name));
}
/**
...
function progressPromptText() {
	return "For any command you expect to run longer than about 30 seconds — including a background job — report live progress to the user: append one JSON event per update to `$DSH_PROGRESS_DIR/<task>.jsonl` (`{\"v\":1,\"task\":\"build\",\"state\":\"running\",\"pct\":40,\"msg\":\"linking\"}`; states are running/done/failed/cancelled, `done`/`total`/`unit` optional), or run `node \"$DSH_PROGRESS_CLI\" emit --task build --pct 40 --msg \"linking\"` (also `done`, `fail`, `list`). Both variables exist in every shell call, and name the task after something recognisable in the command. A long command that reports nothing leaves the user staring at an empty progress panel. Never read the progress file back — it is the human's view.";
}
/**
...
function apply(ctx, rawConfig) {
	...
	ctx.inject(["jobs", "agents"], (withJobs) => {
		const source = { jobsFor: (sessionId) => {
			const agent = withJobs.agents.get(sessionId);
			return agent === void 0 ? void 0 : withJobs.jobs.list(agent);
		} };
		withJobs.effect(() => {
