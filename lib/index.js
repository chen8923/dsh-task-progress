...
*/
function formatThreshold(afterMs) {
	if (afterMs < 6e4) return `${Math.max(1, Math.round(afterMs / 1e3))} s`;
	return `${Math.max(1, Math.round(afterMs / 6e4))} min`;
}
...
function progressSchema() {
	...
	Object.assign(schema, { "~standard": {
		...
		validate: (value) => ({ value: resolveProgressSettings(value) })
	} });
...
//#endregion
export { CONFIG_DEFAULTS, Config, DEFAULT_REMIND_AFTER_MS, PROGRESS_CLI_KEY, PROGRESS_DIR_KEY, PROMPT_ORDER_NAME, PROMPT_SECTION_NAME, SETTINGS_NAMESPACE, apply, createReminderMessage, createTaskStore, dueForReminder, inject, name, progressPromptText, progressSchema, readConfig, registerProgressEnv, registerProgressPrompt, registerProgressReminder, registerStateRoute, reminderText, resolveProgressSettings, stateHandler };
