window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function useSessionJobs(hook, sessionId) {
			if (hook === void 0) return NO_JOBS;
			return hook((state) => sessionId === void 0 ? NO_JOBS : state.jobsBySession?.[sessionId] ?? NO_JOBS);
		}
		//#endregion
  ...
  Add-Content -Encoding utf8 (Join-Path $env:DSH_PROGRESS_DIR build.jsonl)`
		...
		*/
		...
		const zh = {
			...
			"overlay.unreported": "{count} 个后台任务未上报进度",
			"overlay.unreportedOne": "1 个后台任务未上报进度",
			"jobs.heading": "{count} 个后台任务（未上报进度）",
		...
		const en = {
			...
			"overlay.unreported": "{count} background jobs with no progress",
			"overlay.unreportedOne": "1 background job with no progress",
			"jobs.heading": "{count} background job(s) not reporting progress",
