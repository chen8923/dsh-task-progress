...
| **Job snapshots** | `ctx.jobs.list(caller)` — documented as non-consuming snapshots: id, kind, label, lifecycle status, timestamps, owner session. The caller is the **session id** (the registry filters on `job.owner.id === caller`; an agent object matches nothing and answers with the unowned jobs alone), and the call is made for the session whose jobs are being judged, only to decide whether a long job has gone unreported. `ctx.jobs.read()` is **never** called: that cursor is single-consumer and belongs to the model's `job_output` tool. |
| **Browser job roster** | The panels draw DSH's own per-session job roster (`ctx.jobs` on the client — `state` for the snapshot, `watchRows(sessionId)` to keep a session's rows streaming) so a job with no reported progress still has a row: command label, state, elapsed time, exit detail. This is client-side state DSH already pushes to that browser; it never passes through this plugin's route or reaches the Host half. |
